...
---select *,ROW_NUMBER()over(partition by game_id order by game_id) as row_num from tempo order by game_id
---select * from table1 where row_num =2 order by game_id
